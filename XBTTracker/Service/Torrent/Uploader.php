<?php
// src/addons/Harment/XBTTracker/Service/Torrent/Uploader.php
...
class Uploader extends AbstractService
{
     ...
     *
     * @param Upload $torrentFile
     * @return self
     */
    public function setTorrentFile(Upload $torrentFile)
    {
     ...
     * Set the poster file
     *
     * @param Upload|null $posterFile
     * @return self
     */
    public function setPosterFile(Upload $posterFile = null)
    {
     ...
     * Upload the torrent
     *
     * @return \Harment\XBTTracker\Entity\Torrent|string Torrent entity or error message
     */
    public function upload()
    {
        ...
        $torrentData = \Bencode::decode(file_get_contents($torrentFilePath));
    ...
    protected function validateTorrentFile()
    {
        ...
        try {
            $torrentData = \Bencode::decode(file_get_contents($tempPath));
            ...
            // Calculate info hash to check for duplicates
            $infoSection = \Bencode::encode($torrentData['info']);
            $infoHash = strtoupper(bin2hex(sha1($infoSection, true)));
            ...
            if ($existingTorrent) {
                return \XF::phrase('torrent_with_same_info_hash_already_exists');
            }
    ...
    protected function saveTorrentFile()
    {
        ...
        // Save the file
        if (!$this->torrentFile->moveToAbsolutePath($path . '/' . $fileName)) {
            return false;
        }
     ...
     */
    protected function fetchTmdbMetadata(\Harment\XBTTracker\Entity\Torrent $torrent)
    {
        // Check if we already have this TMDB data
        $tmdbData = $this->finder('Harment\XBTTracker:TmdbData')
            ->where('tmdb_id', $this->tmdbId)
            ->fetchOne();
            
        if (!$tmdbData) {
            /** @var \Harment\XBTTracker\Service\Tmdb\Client $tmdbClient */
            $tmdbClient = $this->service('Harment\XBTTracker:Tmdb\Client');
            $tmdbInfo = $tmdbClient->getDetails($this->tmdbId);
            
            if ($tmdbInfo) {
                // Create new TMDB data entity
                /** @var \Harment\XBTTracker\Entity\TmdbData $tmdbData */
                $tmdbData = $this->em()->create('Harment\XBTTracker:TmdbData');
     ...
     * @param \Harment\XBTTracker\Entity\Torrent $torrent
     */
    protected function announceToTracker(\Harment\XBTTracker\Entity\Torrent $torrent)
    {
