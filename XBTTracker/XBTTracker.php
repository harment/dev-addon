...
class XBTTracker extends AbstractAddOn
{
    ...
    public function setupApp(\XF\App $app)
    {
        ...
        $app->templater()->addTemplateDirectory('Harment/XBTTracker', 'public');
        
        // إضافة مسار بحث قوالب المشرف
        $app->templater()->addTemplateDirectory('Harment/XBTTracker', 'admin');
    ...
    protected function setupRoutes(\XF\App $app)
    {
        ...
        if ($router)
        {
            // إضافة مسار /torrents ليتم معالجته بواسطة متحكم Harment\XBTTracker:Torrent
            $router->setPrefixHandlers('torrents', [
                'controller' => 'Harment\XBTTracker:Torrent',
                'context' => 'torrents'
            ]);
        ...
        if ($adminRouter)
        {
            $adminRouter->setPrefixHandlers('torrents', [
                'controller' => 'Harment\XBTTracker:Admin',
                'context' => 'torrents'
            ]);
        }
    }
     ...
     * 
     * @return array Entity structures
     */
     ...
     * 
     * @return array Entity post actions
     */
     ...
     *
     * @param \XF\Entity\User $user User entity
     */
  ...
  /**
    ...
    {
        $xbtStats = \XF::repository('Harment\XBTTracker:Torrent')->getTorrentStats();
